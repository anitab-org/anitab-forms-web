  ...
  render() {
    ...
    return (
      <>
        ...
        <Card.Content>
          ...
          <Card.Description>
            {zulipstat[0] ? (
              <>
                <b>Username: </b>
                {zulipstat[0].zulip_username}
                <br />
                <b>Total Messages: </b>
                {zulipstat[0].total_messages}
                <br />
                <b>First Activity: </b>
                {moment(
                  new Date(zulipstat[0].first_activity),
                  'YYYYMMDD'
                ).fromNow()}
                <br />
                <b>Last Activity: </b>
                {moment(
                  new Date(zulipstat[0].last_activity),
                  'YYYYMMDD'
                ).fromNow()}
                <br />
        ...
        <Card.Content extra>
          <b>Stats Last updated: </b>
          {zulipstat[0]
            ? moment(new Date(zulipstat[0].updated_on), 'YYYYMMDD').fromNow()
            : 'Never'}
          <br />
          {zulipstat[0] ? (
            zulipstat[0].total_messages < 10 ? (
              <span>
                Expected messages to be above 10. Please increase your community
                engagement!
              </span>
            ) : null
          ) : null}
...
export default connect(mapStateToProps, { getZulipStat, updateZulipStat })(
  Zulip
);
